...
## Future Improvements
1. Right now, this tool is simple and something I plan to build on. Linear search works fine because there are not many gyms in Columbus (a few dozen). But if the tool were scaled to the entire United States or globally, using a faster search method like a **hash map**, **binary search**, or even a proper **search engine** would be better.

2. The program currently uses fake gym data. In the future, I want to implement an API (like the Google Maps API) to pull real gym data and possibly use AI or another tool to automatically classify gyms by type.
